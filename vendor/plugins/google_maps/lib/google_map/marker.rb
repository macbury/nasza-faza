module GoogleMap
  ...
  class Marker
    ...
    def to_js
      ...
      options = []
      options << "title: '#{escape_javascript(marker_hover_text)}'" if marker_hover_text
      options << "draggable: true" if self.draggable
      options << "icon: #{icon.dom_id}" if icon
      options << "icon: new GIcon( G_DEFAULT_ICON, '#{marker_icon_path}')" if marker_icon_path

      js << "#{dom_id} = new GMarker( new GLatLng( #{lat}, #{lng} ), {#{options.join(',')}});"
      ...
      js << "#{dom_id}.enableDragging()" if self.draggable
